...
//ma tet3adech ela ma lreciever ya9bel byhom
//asosiation tsyr mara bark.ken taawd taamlha y9olk token already assosiated to the account wykmlch yexecuty lcode
async function associateToken(tokenId, accountId) { 
  ...
  let associateTokenTx = await new TokenAssociateTransaction()
    ...
    .setTokenIds([tokenId])
    .freezeWith(client);
     //eli bech ysigny houwa l receiver
...
async function wipeToken(accountId, tokenId, amount) { //tna7y token laccount
